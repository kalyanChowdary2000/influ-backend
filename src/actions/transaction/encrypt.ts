...
router.post("/", async (req: any, res: any) => {
    try {
        ...
        console.log("user data is ---->", userData.data.name);
        if (userData.data.length != 0) {
            ...
            let encryptedData = ccavenue.getEncryptedOrder({
                ...
                "billing_city": userData.data.city?userData.data.city:"testCity",
                "billing_state": userData.data.state?userData.data.state:"testState",
                "billing_zip": userData.data.pinCode?userData.data.pinCode:123456,
